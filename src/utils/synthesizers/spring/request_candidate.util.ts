...
import type {
  ...
  SpringControllerRequestMatch,
  SpringMethodCallContext,
  SpringParamType,
} from "../../../models/synthesis/spring/request_candidate.model";
import type {
  SynthesisHttpMethod,
...
export type ControllerRequestMatch = SpringControllerRequestMatch;
...
function findMethodCallContext(text: string, methodName: string): SpringMethodCallContext | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const out: SpringMethodCallContext = {
      line: i + 1,
...
function parseEndpointFromAnnotationBlock(
  ...
  classBase: string,
): { method: SynthesisHttpMethod; path: string; source: SynthesisRequestInferenceSource } | null {
  const directMappings: Array<{ rx: RegExp; method: SynthesisHttpMethod }> = [
  ...
  const jaxrsMappings: Array<{ rx: RegExp; method: SynthesisHttpMethod }> = [
...
async function inferEndpointMappingFromInterface(args: {
  ...
  searchRootsAbs: string[];
}): Promise<{ method: SynthesisHttpMethod; path: string; source: SynthesisRequestInferenceSource } | null> {
...
function parseRequestParamsFromController(text: string): SpringParamType[] {
  const out: SpringParamType[] = [];
  const rx =
  ...
  while ((m = rx.exec(text)) !== null) {
    ...
    const entry: SpringParamType = { name, requestName };
...
function parseControllerMethodParams(text: string, callLine: number): SpringControllerParam[] {
  const lines = text.split(/\r?\n/);
  const methodLineIndex =
    findEnclosingMethodStartLine(lines, Math.max(0, callLine - 1)) ?? Math.max(0, callLine - 1);
  ...
  const out: SpringControllerParam[] = [];
  for (const rawParam of rawParams) {
    ...
    let source: SpringControllerParam["source"] = "unknown";
    let requestName = name;
    ...
    const param: SpringControllerParam = {
      name,
      requestName,
      source,
      ...(javaType ? { javaType } : {}),
...
async function findOpenApiOperationByOperationIds(args: {
  searchRootsAbs: string[];
  operationIds: string[];
}): Promise<{ method: SynthesisHttpMethod; path: string } | null> {
  ...
  for (const rootAbs of args.searchRootsAbs) {
    ...
    for (const fileAbs of candidates) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        let method: SynthesisHttpMethod | undefined;
        let pathValue: string | undefined;
        for (let j = i; j >= 0; j--) {
          const l = lines[j] ?? "";
          const methodMatch = l.match(/^\s{4}(get|post|put|patch|delete):\s*$/i);
          if (!method && methodMatch?.[1]) {
            method = methodMatch[1].toUpperCase() as SynthesisHttpMethod;
          }
...
function findCallerMethodsForCallee(args: {
  ...
  const out: SpringCallerMethodCandidate[] = [];
...
async function buildRecipeCandidate(args: {
  ...
  const mappedParams = args.call.argNames
    ...
    .filter((p): p is SpringControllerParam => typeof p !== "undefined");
  ...
  const legacyMatchedParams = args.call.argNames
    ...
    .filter((p): p is SpringParamType => typeof p !== "undefined");
  ...
  const openapiPath = requestParamName
    ...
    : null;
  let method: SynthesisHttpMethod = endpointMapping?.method ?? "GET";
  let requestSource: SynthesisRequestInferenceSource | undefined = endpointMapping?.source;
...
async function buildBestEffortCandidateFromControllerDeclaration(args: {
  ...
  const pseudoCall: SpringMethodCallContext = {
    line: methodLine + 1,
  ...
  const updated: SynthesisRecipeCandidate = {
    ...built.recipe,
