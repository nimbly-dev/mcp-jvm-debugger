...
public final class ProbeHttpServer {
  ...
  private static final class StatusHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
      ...
      if (!ProbeAuth.authorizeObserve(exchange)) {
        writeJson(exchange, 401, "{\"error\":\"unauthorized\",\"scope\":\"observe\"}");
        return;
      }
  ...
  private static final class ResetHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
      ...
      if (!ProbeAuth.authorizeActuate(exchange)) {
        writeJson(exchange, 401, "{\"error\":\"unauthorized\",\"scope\":\"actuate\"}");
        return;
      }
  ...
  private static final class CaptureHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
      ...
      CaptureRecordView capture = ProbeRuntime.getCaptureById(captureId.trim());
      if (capture == null) {
        writeJson(
            exchange,
            404,
  ...
  private static String buildStatusBodyJson(String key) {
    ...
    CapturePreviewView capturePreview = ProbeRuntime.getCapturePreviewForKey(key);
    RuntimeStringSignal applicationType = ProbeRuntime.getApplicationTypeSignal();
    RuntimePortSignal appPort = ProbeRuntime.getAppPortSignal();
  ...
  private static String buildCapturePreviewJson(CapturePreviewView preview) {
  ...
  private static String buildCaptureRecordJson(CaptureRecordView capture) {
    return "{"
  ...
  private static String buildRuntimeStringSignalJson(RuntimeStringSignal signal) {
    if (signal == null) {
      return "{"
          + "\"value\":\"unknown\","
  ...
  private static String buildRuntimePortSignalJson(RuntimePortSignal signal) {
    if (signal == null) {
      return "{"
          + "\"value\":null,"
          + "\"source\":\"runtime_introspection\","
  ...
  private static String buildCaptureValuesJson(List<CaptureValueView> values) {
  ...
  private static String buildCaptureValuesMetaJson(List<CaptureValueView> values) {
    if (values == null || values.isEmpty()) return "[]";
    List<String> rows = new ArrayList<>();
    for (int i = 0; i < values.size(); i++) {
      rows.add("{"
  ...
  private static String buildNullableCaptureValueJson(CaptureValueView value) {
    if (value == null) return "null";
    return "{" + buildCaptureValueBodyJson(value) + "}";
  }

  private static String buildNullableCaptureValueMetaJson(CaptureValueView value) {
    if (value == null) return "null";
    return "{" + buildCaptureValueMetaBodyJson(value) + "}";
  }

  private static String buildCaptureValueBodyJson(CaptureValueView value) {
  ...
  private static String buildCaptureValueMetaBodyJson(CaptureValueView value) {
